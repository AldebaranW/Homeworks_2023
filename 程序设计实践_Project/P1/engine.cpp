...
Engine::Engine(int initial_pet, int enemy_type)
{
    player = new Player(initial_pet);
    switch (enemy_type) {
    case 0:
        enemy = new Enemy(initial_pet);
        break;
    case 1:
        enemy = new Greedy_Enemy(initial_pet);
        break;
    case 2:
        enemy = new Potion_Enemy(initial_pet);
    }
}
...
void Engine::Play()
{
    ...
    while (!end && round <= 100) {
        ...
        Action player_act; 
        do {
            std::cout << "Select your action (1 for skill" << (player->pet_remain > 1 ? ", 2 for change):" : "):") << std::endl;
            std::cin >> action;
        ...
        case 1:
            while (cmd < 1 || cmd > 2) {
                ...
                std::cin >> cmd;
                player_act.power = 20;
                player_act.attack_pt = player->GetPetOnStage().GetAttackPoints();
